...
// Removed direct stats utility import - Hook handles it
...
export default function MDEmployeeManagement() {
    ...
    // Standardized Hook - The "Single Spigot"
    ...
    // Effect to separate active/archived if needed (or API can do it)
    useEffect(() => {
        if (employees.length > 0) {
            // Client side filter for archived (if API returns mixed)
            // or separate API call for archived in future.
            // For now, assuming API returns active by default, 
            // but we can create a separate hook for archived later.
            setArchivedEmployees(employees.filter(e => e.status === 'archived'))
        }
    }, [employees])

    // Fetch Data handled by Hook automatically
