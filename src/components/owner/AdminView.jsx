...
export default function AdminView({ currentUser, searchTerm }) {
    // const [searchTerm, setSearchTerm] = useState('') <-- Removed, using prop
    ...
    const filteredEmployees = useMemo(() => {
        ...
        if (activeTab === 'suspended') {
            ...
            data = data.filter(e => (e.role || '').toLowerCase() === activeTab)
        }
    ...
    const handleDeleteUser = (uid) => {
        showModal({
            type: 'error',
            title: 'Delete User?',
            message: 'This action cannot be undone. The user will be permanently archived.',
            primaryAction: {
                label: 'Delete User',
                onClick: async () => {
                    try {
                        ...
                        closeModal()
                        showModal({ type: 'success', title: 'User Deleted', message: 'User has been successfully archived.', onClose: closeModal })
                    } catch (error) {
    ...
    return (
        <div className="flex flex-col h-full gap-4">
            ...
            {/* Users Table Container - Now flex-1 to fill remaining height */}
            <div className="flex-1 flex flex-col bg-slate-800/50 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden shadow-xl min-h-0">
                {/* Integrated Header with Filters */}
                <div className="p-4 border-b border-white/5 bg-slate-900/30 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-2">
                        <h2 className="text-lg font-semibold text-white">Team Members</h2>
                        <span className="px-2 py-0.5 rounded-full bg-white/5 text-xs text-slate-400 font-medium border border-white/5">
                ...
                <div className="flex-1 overflow-auto relative">
                    {loading ? (
                        <div className="absolute inset-0 flex items-center justify-center text-slate-400">Loading users...</div>
                    ) : (<>
                        ...
                        <div className="md:hidden divide-y divide-white/5">
                            ...
                            {currentData.length === 0 && <div className="p-8 text-center text-slate-500">No data present</div>}
                        </div>
                    </>)}
