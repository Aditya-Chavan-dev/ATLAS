...
ATLAS eliminates human-as-middleware by enforcing an event-driven approval loop with machine-verified timestamps. The system operates three core process flows tied to distinct user roles: Employee, Managing Director (MD), and system automation.

The attendance flow begins when an employee marks attendance via the mobile interface. The system generates a server-side timestamp immune to client manipulation. All attendance is marked as "pending" and triggers an asynchronous notification to all active MDs via Firebase Cloud Messaging. The MD receives a push notification with employee details, reviews the request in the approval queue, and either approves or rejects with a single tap. The system writes the decision with MD attribution and timestamps, then notifies the employee of the outcome. End-to-end latency from submission to approval: median 5-10 minutes for human-reviewed attendance (observed).
...
ATLAS operates within defined boundaries. The system handles attendance marking with manual MD approval, leave request submission and approval, real-time dashboard statistics, and Excel-based reporting. It does not implement shift management (single check-in per day, no clock-in/clock-out pairs), overtime tracking, payroll integration, or complex leave types beyond paid leave and compensatory off.
